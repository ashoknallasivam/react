...
export const FetchEnrollmentTarget = () => (dispatch, getState) => {//move this to tenant actions
    ...
    if (LtoReducers.currentLtoFlag !== "createFlag") {
        axios.get(`${BACKEND_URL}/enrollment-target`, { mode: 'cors' }, config).then((response) => {
            ...
            let enrollmentList = [];
            projectFormReducer.locations.map(itemLoc => {
                //if (!((""+itemLoc.id).includes("c"))) {
                response.data.map(item => {
                    if (itemLoc.id == item.orgId) {
                        enrollmentList.push(item);
                    }
                })
                // }
            })
            if (!objectUtil.isEmpty(enrollmentList)) {
                dispatch(actions.UpdateEnrollmentTarget(enrollmentList));
            }
            // projectInfo[0].lowerLevelOrg.map(itemLoc => {
            //     response.data.map(item => {
            //         if (itemLoc.id == item.orgId)
            //             enrollmentList.push(item);
            //     })
            // })
            // console.log("action",enrollmentList);
            // dispatch(actions.UpdateEnrollmentTarget(enrollmentList));
        })
            .catch(function (error) {
                console.log(error);
...
export const UpdateEnrollmentTarget = (data) => (dispatch, getstate) => {
    ...
    // let editedData = [];
    // getstate().createProj.enrollmentTargetList.map(item=>{
    //     if (item.hasOwnProperty('flag'))
    //         editedData.push(item);
    // })
    // if (!objectUtil.isEmpty(editedData))
    //     dispatch(actions.AddEnrollmentTarget(editedData));
    // else
