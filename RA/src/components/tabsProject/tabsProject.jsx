...
import { EnrollmentTargetModal, EnrollmentTarget } from './tabs';
...
class TabsProject extends Component {
    constructor(props) {
        ...
        // objectUtil.bindAction(HeaderData.studyConfigHeader, "EditColumn", this.deleteStudyConfigGroups);
    }
    handleRoleInput = (e) => {
    ...
    submitStudyConfig = (e) => {
        ...
        this.props.studyConfigList.map(item => {
            if (item.ltoId == this.props.selectedCurrentLTO) {
                item.groups.map(res => {
                    sum = sum + parseFloat(res.ratio);
                })
            }
        ...
        if (sum > 1)
            ...
            this.props.actions.AddStudyConfig(this.inputData);
            this.props.actions.AddFinalStudyConfig();
            this.inputData = {};
        }
    }
    deleteStudyConfig = (e) => {
    ...
    deleteStudyConfigGroups = () => {
        ...
        if (!(objectUtil.isEmpty(selectedData))) {
            ...
            window.Materialize.toast(localConstant.warningMessages.DELETE_ROWS, 1000);
    }
    ...
    enrollmentDateHandler = (data,selectedYear) => {
        this.month = data
    ...
    submitEnrollmentTargetModal = (e) => {
        ...
        if (this.updatedData && !(objectUtil.isEmpty(this.updatedData))) {
            ...
            window.Materialize.toast("Please enter the feilds");
        }
    }
    ...
    handleRoleDropdown = (e) => {
        ...
        let filteredRolesData = this.props.ApplicationMode == 'VIEW' ?
            ...
            this.props.ApplicationMode == 'EDIT' && flag ?  filteredRoles[0] : this.props.ApplicationMode == 'EDIT' && !flag ? 
            this.roleBodyCreate(description, id, isAssignable.data[0], isAutoAccess.data[0], isAutoAssignOnIntake.data[0], name)
            : filteredRoles[0];
    ...
    render(){
        ...
        const { functionsList, studyConfigList, finalStudyConfigData, enrollmentTargetData, locations } = this.props;
        ...
        locations.map(item => {
            if (item.id == this.props.selectedCurrentLTO)
                if (item.hasOwnProperty('flag')) {
                    ...
                    finalStudyConfigData.map(item => (item.ltoId == this.props.selectedCurrentLTO) ? this.isLocationExist = true : this.isLocationExist = false);
                }
        })
                    ...
                    <Tabs>
                        ...
                        <TabPanel>
                            ...
                            <StudyConfig
                                ...
                                finalStudyConfigData={finalStudyConfigData}
                            ...
                            <TabPanel>
                                <Row className="m-0 mt-3 pl-2 CreateProjectTabContent">
                                    <div className='col s12 m12 l12 xl12 mb-2'>
                                    {
                                        ...
                                        <select className="browser-default col input-field s12 m3 l3 x3 mt-1 role_select_box"
                                        onChange={this.handleRoleDropdown}>
                                            <option disabled={this.state.disabledSelectRole} key={this.state.roleDefaultValue} defaultValue={this.state.roleDefaultValue}>Select Role</option>
                                            {
