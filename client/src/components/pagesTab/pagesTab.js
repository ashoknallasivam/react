  ...
  constructor(props) {
	...
	getData(pageJSON){
	  ...
	  //Send the data to store via action 
		...
		//console.log(this.state.tenantId , pageJSON);
    ...
    render(){
		...
		if (this.state.addForm == 'Edit')
		{
			dynamicForm = <PageFields sendData={this.getData}/> ;
		}
				...
				<Col className="z-depth-8 mr-0" s={12} m={6} l={4} xl={6} >
					<div style={{ maxWidth: "1400px", maxHeight: "100%" }}>
							<form className="col-md-4" onSubmit={this.handleSubmit} >
								...
								{viewOnly == false ? (
					                 <Row className="left submit-container">
									 <Col className="input-field p-0" s={12}>
					 ...
					 <Row>
					  ...
					  {viewOnly == false ? (	
						  <Col s={2} className='z-depth-8 mr-0'>
							<a class="btn-floating btn-small grey" onClick={this.CreatePage}><i class="material-icons">add</i></a>
						  </Col>
					   ) : ('')
      				  }
					 </Row>
