...
import { Modal as Modal1 } from 'react-materialize';
...
class TabsProject extends Component {
    ...
    addDescAndBlock = () => {
        if (!objectUtil.isEmpty(this.inputData.description) || !objectUtil.isEmpty(this.inputData.blockSize)) {
            this.flagStatus();
        }
    }
    flagStatus = () => {
    ...
    submitStudyConfigGroups = (e) => {
        e.preventDefault();
        if (!(objectUtil.isEmpty(this.updatedData))) {
            this.flagStatus();
    ...
    getEnrollmentTargetData = () => {
        let enrollmentData = [];
        this.props.enrollmentTargetData.map(item => {
            if (item.orgId == this.props.selectedCurrentLTO) {
                enrollmentData.push(item);
            }
        })
    ...
    proceedChange = (e) => {
        if (this.formDataChanged) {
            this.setState({ tempSelectedTabIndex: e });
            $('#lostDataRole').modal('open')
            return false;
        } else {
    ...
    handleLostDataAlert = () => {
        this.setState({ selectedTabIndex: this.state.tempSelectedTabIndex });
        this.formDataChanged = false;
        $('#lostDataRole').modal('close')
    }
    render() {
        ...
        this.studyConfigAddButtons = [
            {
                ...
                showbtn: true,
                disabled: (this.state.isStudyGroupEdit && objectUtil.isEmpty(this.updatedData)) ? true : false
            },
            {
        ...
        this.enrollmentButtons = [
            {
                ...
                btnClass: 'btn_secondary mr-1',
                showbtn: true,
                disabled: (this.state.isEnrollmentEdit && objectUtil.isEmpty(this.updatedData)) ? true : false
            },
            {
                name: localConstant.commonConstants.CANCEL,
        ...
        return (
            <div className='tabs-project' id="tabsProject">
                {this.props.selectedCurrentTTO != "" && this.props.orgDeletedStatus == false ?
                    <Tabs selectedIndex={this.state.selectedTabIndex} onSelect={$event => this.proceedChange($event)}>
                        ...
                        <TabPanel>
                            ...
                            <EnrollmentTarget
                                enrollmentTargetData={this.getEnrollmentTargetData()}
                                selectedLTO={this.props.selectedCurrentLTO}
