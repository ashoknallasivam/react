...
router.post('/role', (req, res) => {
    ...
    axios.post(`${config.RAPTER_URL}/role`, inpParam, requestOptions).then(function (response) {
...
router.get('/role', (req, res) => {
    ...
    axios.get(`${config.RAPTER_URL}/role`, requestOptions).then(function (response) {
        res.status(200).send(response.data);
    }).catch(error => {
        logging.applogger.error(error);
        res.status(500).send({ code: error.response.status, status: error.response.statusText, messages: error.response.data.error });
...
router.get('/role/:id', (req, res) => {
    ...
    if (inpParam !== undefined || Object.keys(inpParam).length !== 0) {
        axios.get(`${config.RAPTER_URL}/role/` + inpParam.id, requestOptions).then(function (response) {
            res.status(200).send(response.data);
        }).catch(error => {
            logging.applogger.error(error);
            res.status(500).send({ code: error.response.status, status: error.response.statusText, messages: error.response.data.error });
...
router.put('/role/:id', (req, res) => {
    ...
    axios.put(`${config.RAPTER_URL}/role/` + inpParam.id, req.body, requestOptions).then(function (response) {
        res.status(200).send(response.data);
    }).catch(error => {
        logging.applogger.error(error);
        res.status(500).send({ code: error.response.status, status: error.response.statusText, messages: error.response.data.error });
...
router.delete('/role/:id', (req, res) => {
    ...
    axios.delete(`${config.RAPTER_URL}/role/` + inpParam.id, requestOptions).then(function (response) {
