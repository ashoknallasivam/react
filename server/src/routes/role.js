...
router.post('/role', (req, res) => {
    ...
    roleBiz.createRole(requestOptions,inpParam).then(response=>{
...
router.get('/role/:id', (req, res) => {
    ...
    if (inpParam !== undefined || Object.keys(inpParam).length !== 0) {
        roleBiz.getRole(requestOptions, inpParam.id).then(response=>{
            if(response.status===200){
                res.status(200).send(response.data);
            }else{
                res.status(500).send(response)
...
router.put('/role/:id', (req, res) => {
    ...
    roleBiz.updateRole(requestOptions, inpParam.id, req.body).then(response=>{
        if(response.status===200){
            res.status(200).send(response.data);
        }else{
            res.status(500).send(response)
...
router.delete('/role/:id', (req, res) => {
    ...
    roleBiz.deleteRole(requestOptions, inpParam.id).then(response =>{
        if(response.status===200){
            res.status(200).send(response.data);
        }else{
            res.status(500).send(response)
